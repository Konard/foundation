...
def get_categories(title):
    url = f'https://en.wikipedia.org/w/api.php?action=query&prop=categories&titles={title}&cllimit=500&format=json'
    headers = {'User-Agent': USER_AGENT}
    response = requests.get(url, headers=headers)
    try:
        response.raise_for_status()
    ...
    for page_id in pages:
        if 'categories' in pages[page_id]:
            for category in pages[page_id]['categories']:
...
def main():
    ...
    for article in top_articles:
        ...
        if any('programming' in category.lower() for category in categories):
    ...
    with open(f'top_{limit}_programming_wikipedia_articles.json', 'w', encoding='utf-8') as f:
        json.dump(programming_articles, f, ensure_ascii=False, indent=4)
    
    print(f"Top {limit} Wikipedia articles about programming and their translations have been saved to 'top_{limit}_programming_wikipedia_articles.json'.")
